...
dst = f'/users/jdt0025/scratch/Erie/active_learning/active_learning'
pretrained_model_path = os.path.join(f'figures/AL-{al_method}/iter_{al_iter}/best_model.pt') #os.path.join(root, 'best_model.pt')

val_dataset = ErieParcels(os.path.join(root, 'parcels_cleaned'), os.path.join(root, f'active_learning/active_learning/test.csv'), return_id=True, split=None)
...
os.mkdir(os.path.join(dst, al_method))
os.mkdir(os.path.join(dst, al_method, 'active'))
os.mkdir(os.path.join(dst, al_method, 'inactive'))
...
for X, y, _id in tqdm(val_dataset):
    ...
    if pred == 1 and y == 1:
        ...
        img = F.to_pil_image(X.cpu())
        img.save(os.path.join(dst, al_method, 'inactive', f'{_id}.png'))
        out['FP'].append(_id)   
    elif pred == 0 and y == 1:
        img = F.to_pil_image(X.cpu())
        img.save(os.path.join(dst, al_method, 'active', f'{_id}.png'))
        out['FN'].append(_id)   
